...
class RSAKey( object ):
	...
	def dump( self, filename ):
		try:
			handle = open( filename+".private", 'w' )
			publica = self.meta.copy()
 			del publica["pub_key"]
			pickle.dump( publica, handle )
			handle.close( )
			print('escribiendo la clave publica en: ' + str( filename )+".private")
			
			handle = open( filename+".public", 'w' )
			privada = self.meta.copy()
 			del privada["priv_key"]
			pickle.dump( privada, handle )
			handle.close( )
			print('escribiendo la clave privada en: ' + str( filename )+".public")
